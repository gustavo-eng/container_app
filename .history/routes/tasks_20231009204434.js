...
router.get('/files',  function(req, res, next) {
  ...
  const data = fs.readFileSync('./files/teste.json',   (err, data) => {
    if(err) {
  ...
  try {
    let novoElemento = { "id": 51, "name": "Novo Elemento" };
    let jsonData = JSON.parse(data)

    jsonData.push(novoElemento)

    const novoConteudoJSON = JSON.stringify(jsonData, null, 4);

    fs.writeFile('./files/teste.json', novoConteudoJSON, 'utf8', (writeErr) => {
      if (writeErr) {
