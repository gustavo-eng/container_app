...
router.delete("/:id", controlaAcesso, TaskValidator.validateId, function(req, res){
  if(fs.existsSync('./files/data.json')) {
    ...
    fs.unlink('./files/data.json', (err) => {
      if(err) {
        return res.json({ status: false, msg: "Falha ao excluir o arquivo" });
      } else {
        return res.json({ status: true, msg: "Arquivo excluído com sucesso" });
      }
    })
  } else {
    return res.json({ status: false, msg: "Arquivo não encontrado" });
