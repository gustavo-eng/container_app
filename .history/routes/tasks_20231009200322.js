...
router.get('/files',  function(req, res, next) {
  ...
  const file1 = fs.readFileSync('./files/teste.json',   (err, data) => {
    console.log('fs.readFile (data) -->  ')
    console.logO(data)

  })

  const file2 =  fs.readFileSync('./arquivos/teste.txt',   (err, data) => {
    console.log('fs.readFile (data) -->  ')
    console.logO(data)

  })
  const modified = JSON.parse(file1)
  console.log('file1')
  console.log(JSON.parse(file1))
  console.log('modified -->')
  console.log(modified.push({ "id": 51, "name": "gustavo" }))
